...
export default function EditLaptopModal({ laptop, existingModels = [], existingVendors = [], existingRates = [], onClose }) {
  ...
  const processUpdate = async () => {
    ...
    setLoading(true);
    try {
      await updateLaptop(laptop.id, { ...form, model: form.model.trim(), serialNo: form.serialNo.trim(), hrRefNumber: form.hrRefNumber.trim(), ratePerMonth: Number(form.ratePerMonth) || 0 });
